...
//----------------------------- MODIFICACION ---------------------------------
void TAlmacen::AniadirPedido(TPedido p)
{
    //Recorremos la cola de pedidos entera para ver si el CodProd está y es de la misma tienda, si es asi le sumamos la cantidad nueva al ya existente
    int i = 1;
    TPedido paux; //Necesitamos un tpedido auxiliar para poder comparar con el que me pasa por parametro
    bool encontrado = false;

    while(i <= Pedidos.longitud())
    {
        paux = Pedidos.primero(); //Guardamos en paux el primer pedido de la cola

        //Hacemos las dos comparaciones: Si es el mismo CodProd
        if(strcmp(paux.CodProd,p.CodProd)==0)
        {
            //Ahora vemos si es de la misma tienda
            if(strcmp(paux.Nomtienda,p.Nomtienda)==0)
            {
                paux.CantidadPed += p.CantidadPed; //Le añadimos a paux la cantidad que tenia + la de p
                encontrado = true;
            }

        }
        i++;
        Pedidos.desencolar(); //Desencolamos y encolamos el elemento p al final, para no perderlo
        Pedidos.encolar(paux);
    }///FIN DEL BUCLE WHILE

    //Si ha encontrado al producto, no lo añadimos al final, en caso contrario si
    if(!encontrado)
        Pedidos.encolar(p);
}
