...
void TestBusqueda::comprobarMetodosBusqueda()
{
	...
	for (int metodo = 0; metodo < nombreAlgoritmo.size(); metodo++)
	{
		...
		if (pos == -1)
			...
			cout << "	->  Posicion de " << key << " buscado con el metodo " << nombreAlgoritmo[metodo] << ": " << pos+1;
		cout << endl;
		v->vaciar();
		delete v;
		system("pause");
...
void TestBusqueda::casoMedio(int metodo)
{
	...
	{
		Graficas G;
		cout << "\n\nIndique la complejidad del metodo (SecuencialIt(S), BinarioIt(B), InterpolacionIt(I): ";
		cin >> complejidad;
		complejidad = toupper(complejidad);
		switch (complejidad)
		{
		case 'S': G.generarGraficaMEDIO(nombreAlgoritmo[metodo], N);
			break;

		case 'B': G.generarGraficaMEDIO(nombreAlgoritmo[metodo], logN);
			break;

		case 'I': G.generarGraficaMEDIO(nombreAlgoritmo[metodo], loglogN);
			break;
		}
