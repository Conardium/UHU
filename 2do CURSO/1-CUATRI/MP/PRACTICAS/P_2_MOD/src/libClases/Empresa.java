...
public class Empresa implements Cloneable{
	...
	//Crear el metodo ganancia que calcula la ganancia para los Clientes de TP antes de un año pasado por parametro
	public float ganancia(int anio) {
		float total=0;
		
		for(int i=0; i<nClientes; i++) {
			Fecha f = clientes[i].getFechaNac();
			//Miramos si el cliente de la posicion i-ésima es de tipo Cliente TP y su año de nacimiento (FechaNac)
			//sea menor que el año que pasamos por parametro
			if(clientes[i].getClass()==ClienteTarifaPlana.class && f.EsMenor(f.getAnio(), anio)==1)
